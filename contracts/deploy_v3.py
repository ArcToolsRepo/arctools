...
vault_c = solcx.compile_source(open("ArcPad.sol").read(), output_values=["abi", "bin"], optimize=True, optimize_runs=200, via_ir=True)["<stdin>:ArcRewardsVault"]
...
send(pad.functions.setMinTarget(int(0.3e18)), 60_000); print("minTarget = 0.3 (test) — raise after tests")

json.dump({"launchpad": PAD, "vault": VAULT, "abi": v3["abi"], "vault_abi": vault_c["abi"]}, open("arcpad_deploy_v3.json", "w"), indent=1)
