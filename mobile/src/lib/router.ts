...
export type Route =
  ...
  | { name: "bridge" } | { name: "locker" } | { name: "predict" } | { name: "market" } | { name: "cross"; token?: string; sym?: string } | { name: "rewards" } | { name: "alerts" } | { name: "profile"; wallet?: string } | { name: "settings" }
...
export function parse(hash: string): Route {
  ...
  switch (seg[0]) {
    ...
    case "cross": return { name: "cross", token: seg[1]?.toLowerCase(), sym: q.get("sym") ?? undefined };
    case "insiders": case "launchpad": case "locker": case "predict": case "market": case "pay": case "referrals": case "bridge": case "rewards": case "alerts":
