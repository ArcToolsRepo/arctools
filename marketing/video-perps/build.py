...
XF = 0.5                     # crossfade
PAD = 0.4                    # tail after narration
VO_IN = 0.3                 # narration starts this late into a segment
...
for i, seg in enumerate(segs):
    ...
    if seg["kind"] == "robot":
        vid = f"robot_{seg['robot']}.mp4"; D = min(dur(vid), max(3.4, vo + VO_IN + PAD))
    else: D = vo + VO_IN + PAD
...
subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", "video_nosub.mp4", "-i", "audio.m4a", "-vf", f"subtitles=captions.ass:fontsdir={FDIR}", "-c:v", "libx264", "-preset", "medium", "-crf", "18", "-pix_fmt", "yuv420p", "-c:a", "copy", "-shortest", "-movflags", "+faststart", "arcperps-30s.mp4"], check=True)
