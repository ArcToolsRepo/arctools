...
# left column: robot
LEFT_W = 600
rb = robot.resize((int(robot.width * 800 / robot.height), 800), Image.LANCZOS)
if rb.width > LEFT_W: rb = rb.resize((LEFT_W, int(rb.height * LEFT_W / rb.width)), Image.LANCZOS)
bg.alpha_composite(rb, (20 + (LEFT_W - rb.width) // 2, H - rb.height - 60)); d = ImageDraw.Draw(bg)

# right column: screenshot card
cx0 = 660; cw = W - 56 - cx0
shot = Image.open('crop_firstcall.png').convert('RGBA')
sw = min(cw - 16, 800); shot = shot.resize((sw, int(shot.height * sw / shot.width)), Image.LANCZOS)
sy = 300
cxs = cx0 + (cw - sw - 16) // 2
...
bg.convert('RGB').save('../34-first-call-arc.png', quality=95); print('saved')
