...
export const Route = createFileRoute("/leaderboard2")({ component: Leaderboard });
...
function Leaderboard() {
  ...
  return (
    <main className="arc-dsp" style={{ minHeight: "100dvh" }}>
      <DsRail />
      <section className="arc-dsp__body" style={{ display: "grid", gap: 16, maxWidth: 1180, paddingBottom: 80, paddingTop: 118 }}>
      <header>
      ...
      <div style={{ border: "1px solid var(--arc-line)", borderRadius: 12, overflow: "hidden" }}>
        ...
        {!loading && !rows.length && (
          <p className="arc-mono" style={{ color: "var(--arc-muted)", fontSize: 12, padding: 16 }}>
            Nobody qualifies yet. <Link style={{ color: "var(--arc-cobalt)" }} to="/profile2">Create the first profile</Link>.
          </p>
        )}
        {rows.map((r, i) => (
          <Link
