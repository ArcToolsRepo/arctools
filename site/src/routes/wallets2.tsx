...
export const Route = createFileRoute("/wallets2")({
    ...
    <main className="arc-dsp" style={{ minHeight: "100dvh" }}>
      ...
      <section className="arc-dsp__body" style={{ maxWidth: 1100, paddingTop: 112 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "18px 0 6px" }}>
          ...
          <Link className="arc-mono" style={{ alignSelf: "center", color: "var(--arc-cobalt)", fontSize: 12 }} to="/insiders2">pick from the insider leaderboard →</Link>
        </div>
        ...
        {wallets.length === 0 && (
          <div style={{ border: "1px dashed var(--arc-line)", borderRadius: 10, color: "var(--arc-muted)", padding: 28, textAlign: "center" }}>
            No wallets yet. Add one above, or click <strong>watch</strong> next to any wallet on <Link style={{ color: "var(--arc-cobalt)" }} to="/intel2">Intel</Link> or a token page.
          </div>
        )}
                  ...
                  <div>
                    ...
                    {c?.positions.length ? c.positions.slice(0, 6).map((p) => (
                      <div className="arc-mono" key={p.token} style={{ display: "flex", fontSize: 12, gap: 8, justifyContent: "space-between", padding: "3px 0" }}>
                        <span><Link params={{ ca: p.token }} preload="intent" style={{ color: "var(--arc-ink)", fontWeight: 700, textDecoration: "none" }} to="/token2/$ca">{p.symbol ?? short(p.token)}</Link> <Link params={{ ca: p.token }} preload="intent" style={{ color: "var(--arc-cobalt)", fontSize: 11, textDecoration: "none" }} to="/token2/$ca">chart ↗</Link></span>
        ...
        <div className="arc-toasts" style={{ bottom: 14, display: "flex", flexDirection: "column", gap: 6, maxWidth: "calc(100vw - 28px)", pointerEvents: "none", position: "fixed", right: 14, width: 270, zIndex: 60 }}>
          {live.map((t) => { const up = t.side === "buy"; const col = up ? "var(--arc-up)" : "#f0534f"; return (
            <Link className="arc-toast" key={t.key} params={{ ca: t.token }} style={{ alignItems: "center", background: "rgba(10,14,22,0.94)", border: "1px solid rgba(255,255,255,0.08)", borderLeft: `3px solid ${col}`, borderRadius: 10, boxShadow: "0 6px 22px rgba(0,0,0,0.4)", color: "var(--arc-ink)", display: "flex", gap: 8, padding: "7px 10px", pointerEvents: "auto", textDecoration: "none" }} to="/token2/$ca">
