...
function Trade() {
  ...
  // ?pad=foci.family — deep link into one launchpad's tokens (shared from the token page, X posts, the app)
  useEffect(() => {
    const want = new URLSearchParams(window.location.search).get("pad");
    if (want) setPadF(want);
  }, []);
  useEffect(() => {
    let alive = true;
                ...
                <table style={{ borderCollapse: "collapse", width: "100%" }}>
                  ...
                  <tbody>
                    ...
                    {positions.map((p) => { const t = byToken.get(p.token.toLowerCase()); const sym = p.symbol ?? t?.symbol ?? short(p.token); return (
                      <tr key={p.token}>
                        <td className="arc-td"><Link params={{ ca: p.token }} preload="intent" style={{ color: "var(--arc-ink)", textDecoration: "none" }} to="/token/$ca"><Logo t={{ logo: t?.logo, symbol: sym }} /><strong>{sym}</strong></Link></td>
