...
export const Route = createFileRoute("/token2/$ca")({
...
function DsSplit({ label, value, aLabel, aValue, bLabel, bValue, ratio, noBar }: {
  label: string; value: string; aLabel: string; aValue: string; bLabel: string; bValue: string; ratio: number; noBar?: boolean;
}) {
  return (
    <div className="arc-ds__row">
      <div className="arc-ds__rowmain"><span>{label}</span><b>{value}</b></div>
      <div className="arc-ds__rowside">
        <div><span>{aLabel}</span><b>{aValue}</b></div>
        <div className="arc-ds__right"><span>{bLabel}</span><b>{bValue}</b></div>
        {!noBar && <div className="arc-ds__bar"><i style={{ width: `${Math.round(ratio * 100)}%` }} /></div>}
      </div>
    </div>
  );
}
...
function TokenPage() {
  // PREVIEW ROUTE: same data and logic as /token/$ca, DexScreener-style arrangement. The live page is untouched.
  ...
  // tradability verdict: the same probe the Terminal uses for its red rows
  const [sim, setSim] = useState<{ verdict: string; keep_bps: number; detail?: string } | null>(null);
  const [faved, setFaved] = useState(false);
  useEffect(() => { try { setFaved(((JSON.parse(localStorage.getItem("arctools_favs") ?? "[]") as string[]) || []).includes(ca)); } catch { /* ignore */ } }, [ca]);
  useEffect(() => {
    let alive = true;
    fetch(`${BOT_API}/api/sim?token=${ca}`).then((r) => r.json())
      .then((j) => { if (alive && j?.verdict) setSim(j); })
      .catch(() => { /* a missing verdict shows nothing, never a false "safe" */ });
    return () => { alive = false; };
  }, [ca]);
  ...
  // the stats endpoint gives 24h totals but no buy/sell split, so the split is measured on the trades the page
  // already holds and labelled as the sample it is — never presented as a 24h figure
  const flowSample = useMemo(() => {
    const b = effTrades.filter((t) => t.side === "buy").reduce((a, t) => a + (t.usdc || 0), 0);
    const sll = effTrades.filter((t) => t.side === "sell").reduce((a, t) => a + (t.usdc || 0), 0);
    return { buy: b, n: effTrades.length, sell: sll };
  }, [effTrades]);
    ...
    <main className="arc-dsp">
      ...
      <section className="arc-dsp__body arc-token">
        ...
        <div className="arc-token__head">
          ...
          {/* KPI row removed: the stat column carries these figures */}
                  </div>
        ...
        <div style={{ border: "1px solid var(--arc-line)", marginTop: 14 }}>
          ...
          <div className="arc-ds">
            <div className="arc-ds__tiles">
              <div className="arc-ds__tile"><span>PRICE USD</span><b>{price !== null ? `$${price < 0.01 ? price.toFixed(8) : price.toFixed(5)}` : "—"}</b></div>
              <div className="arc-ds__tile"><span>PRICE {pairSym}</span><b>{price !== null ? (price < 0.01 ? price.toFixed(8) : price.toFixed(5)) : "—"}</b></div>
            </div>
            <div className="arc-ds__tiles arc-ds__tiles--3">
              <div className="arc-ds__tile"><span>LIQUIDITY</span><b>{money(liquidity, 0)}</b></div>
              <div className="arc-ds__tile"><span>FDV</span><b>{mcap !== null ? money(mcap, 0) : "—"}</b></div>
              <div className="arc-ds__tile"><span>MKT CAP</span><b>{mcap !== null ? money(mcap, 0) : "—"}</b></div>
            </div>
            <div className="arc-ds__tiles arc-ds__tiles--4">
              {(["5m", "1h", "6h", "24h"] as const).map((k) => (
                <div className="arc-ds__tile arc-ds__tile--c" key={k}>
                  <span>{k.toUpperCase()}</span>
                  <b style={{ color: (eff.change[k] ?? 0) >= 0 ? "var(--arc-up)" : "var(--arc-down)" }}>{pct(eff.change[k])}</b>
                </div>
              ))}
            </div>
            <div className="arc-ds__split">
              <DsSplit label="TXNS" value={String(eff.txns)} aLabel="BUYS" aValue={String(buys)} bLabel="SELLS" bValue={String(sells)} ratio={buys + sells > 0 ? buys / (buys + sells) : 0.5} />
              <DsSplit
                aLabel={`BUY (${flowSample.n})`}
                aValue={money(flowSample.buy, 0)}
                bLabel="SELL"
                bValue={money(flowSample.sell, 0)}
                label="VOLUME 24H"
                ratio={flowSample.buy + flowSample.sell > 0 ? flowSample.buy / (flowSample.buy + flowSample.sell) : 0.5}
                value={money(eff.vol24, 0)}
              />
              <DsSplit label="TRADERS" value={String(eff.traders24)} aLabel="HOLDERS" aValue={holderCount != null ? String(holderCount) : "—"} bLabel="LAUNCHPAD" bValue={info.launchpad ?? "—"} ratio={0.5} noBar />
            </div>
            <div className="arc-ds__acts">
              <button
                className="arc-ds__act"
                onClick={() => {
                  // same store the Terminal's star uses, so a token starred here shows there
                  try {
                    const key = "arctools_favs";
                    const cur = new Set<string>(JSON.parse(localStorage.getItem(key) ?? "[]") as string[]);
                    if (cur.has(ca)) cur.delete(ca); else cur.add(ca);
                    localStorage.setItem(key, JSON.stringify([...cur]));
                    setFaved(cur.has(ca));
                  } catch { /* private mode: the button simply does nothing */ }
                }}
                type="button"
              >{faved ? "★ In watchlist" : "☆ Watchlist"}</button>
              <a className="arc-ds__act" href={`https://t.me/ArcToolsBuyBot?start=alert_${ca}`} rel="noreferrer" target="_blank">🔔 Alert</a>
            </div>
            {sim && sim.verdict !== "error" && (
              <div className={"arc-ds__safety" + (sim.verdict === "trap" ? " is-bad" : "")}>
                <p className="arc-mono">SAFETY</p>
                <div><span>Sell simulation</span><b style={{ color: sim.verdict === "ok" ? "var(--arc-up)" : sim.verdict === "trap" ? "var(--arc-down)" : "#f5c542" }}>
                  {sim.verdict === "ok" ? `passed · ${(sim.keep_bps / 100).toFixed(0)}% round trip` : sim.verdict === "thin" ? `thin pool · ${(sim.keep_bps / 100).toFixed(0)}%` : sim.verdict === "no_route" ? "no pool reachable" : `failed · ${sim.detail ?? "cannot sell"}`}
                </b></div>
                <div><span>Launchpad</span><b>{info.launchpad ?? "—"}</b></div>
                <div><span>Holders</span><b>{holderCount != null ? holderCount : "—"}</b></div>
              </div>
            )}
          </div>
