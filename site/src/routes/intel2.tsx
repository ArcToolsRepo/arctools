...
export const Route = createFileRoute("/intel2")({
  head: () => ({
    meta: [
...
function Intel() {
  ...
  return (
    <main className="arc-dsp" style={{ minHeight: "100dvh" }}>
      <DsRail />
      <section className="arc-dsp__body" style={{ maxWidth: 1280, paddingTop: 130 }}>
        ...
        <div style={{ display: "grid", gap: 18, gridTemplateColumns: "1fr" }}>
                ...
                <tbody>
                  {(whales?.rows ?? []).sort((a, b) => b.ts - a.ts).map((r) => (
                    <tr key={r.tx + r.ts}>
                      ...
                      <td style={td}><a href={`/token2/${r.token}`} style={{ color: "var(--arc-ink)" }}>${r.symbol ?? short(r.token)}</a> <QuickBuy compact symbol={r.symbol ?? short(r.token)} token={r.token} /></td>
              ...
              {(kolFeed?.rows ?? []).slice(0, 40).map((m) => (
                <div key={m.tweet_id} style={{ border: "1px solid var(--arc-line)", borderRadius: 6, padding: "8px 10px" }}>
                  <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: 8, fontSize: 12.5 }}>
                    <a href={`/token2/${m.token}`} style={{ color: "var(--arc-ink)", fontWeight: 700, textDecoration: "none" }}>${m.symbol ?? short(m.token)}</a>
          ...
          {show("insiders") && <div style={card}>
            <Title right={<><a href="/insiders2" style={{ color: "var(--arc-cobalt)" }}>leaderboard</a> · <a href="https://t.me/ArcToolsInsiders" rel="noreferrer" style={{ color: "var(--arc-cobalt)" }} target="_blank">channel</a></>} caption="Latest trades of the leaderboard wallets. Copy sends the wallet to the sniper as a copy-trade target.">INSIDER ACTIVITY · top-100 by 30d PnL</Title>
            <div className="arc-scroll" style={{ maxHeight: 460, overflow: "auto" }}>
              <table style={{ borderCollapse: "collapse", width: "100%" }}>
