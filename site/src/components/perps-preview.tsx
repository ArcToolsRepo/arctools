...
export function PerpsPreview() {
  ...
  useEffect(() => { setCandles([]); void load(); const id = setInterval(load, 15_000); const t = setInterval(() => setNow(new Date()), 1000); return () => { clearInterval(id); clearInterval(t); }; }, [load]);
    ...
    <div className="arc-mono" style={{ display: "grid", gap: 8, fontSize: 12 }}>
      ...
      <div style={{ ...cell, alignItems: "center", display: "flex", flexWrap: "wrap", gap: 22, padding: "10px 14px", position: "relative" }}>
        <button onClick={() => setPick((v) => !v)} style={{ alignItems: "center", background: "transparent", border: 0, color: "var(--arc-ink)", cursor: "pointer", display: "flex", gap: 10, padding: 0 }} type="button">
          <span style={{ alignItems: "center", background: "#0e1118", border: `1px solid ${C.line}`, borderRadius: "50%", display: "inline-flex", height: 28, justifyContent: "center", width: 28 }}>{m.sym[0]}</span>
      ...
      <div style={cell}>
        ...
        <div style={{ overflowX: "auto", padding: 8 }}>
          {tab === "pos" && <table style={{ borderCollapse: "collapse", width: "100%" }}><thead><tr style={{ color: C.muted, textAlign: "left" }}>{["Coin", "Size", "Position Value", "Entry Price", "Mark Price", "PNL (ROE %)", "Liq. Price", "Margin", "Funding", "TP/SL", ""].map((h) => <th key={h} style={{ fontWeight: 400, padding: "4px 8px" }}>{h}</th>)}</tr></thead><tbody>
            <tr style={{ borderTop: `1px solid ${C.line}` }}><td style={{ padding: "6px 8px" }}><b>NVDA</b> <span style={{ color: C.up }}>3x</span></td><td style={{ color: C.up, padding: "6px 8px" }}>4.01 NVDA</td><td style={{ padding: "6px 8px" }}>$900.00</td><td style={{ padding: "6px 8px" }}>221.10</td><td style={{ padding: "6px 8px" }}>{MARKETS[0] && sel === 0 && mark ? px(mark) : "223.93"}</td><td style={{ color: C.up, padding: "6px 8px" }}>+$11.46 (+3.8 %)</td><td style={{ padding: "6px 8px" }}>158.45</td><td style={{ padding: "6px 8px" }}>$300.00 (isolated)</td><td style={{ padding: "6px 8px" }}>−$0.04</td><td style={{ padding: "6px 8px" }}>240.00 / 205.00</td><td style={{ padding: "6px 8px" }}><button style={{ ...seg(false), padding: "3px 10px" }} type="button">Close</button></td></tr>
            <tr style={{ borderTop: `1px solid ${C.line}` }}><td style={{ padding: "6px 8px" }}><b>ARGUS</b> <span style={{ color: C.down }}>2x</span></td><td style={{ color: C.down, padding: "6px 8px" }}>−21,660 ARGUS</td><td style={{ padding: "6px 8px" }}>$400.00</td><td style={{ padding: "6px 8px" }}>0.01930</td><td style={{ padding: "6px 8px" }}>{sel === 5 && mark ? px(mark) : "0.01850"}</td><td style={{ color: C.down, padding: "6px 8px" }}>−$2.70 (−1.3 %)</td><td style={{ padding: "6px 8px" }}>0.02750</td><td style={{ padding: "6px 8px" }}>$200.00 (isolated)</td><td style={{ padding: "6px 8px" }}>+$0.11</td><td style={{ padding: "6px 8px" }}>— / —</td><td style={{ padding: "6px 8px" }}><button style={{ ...seg(false), padding: "3px 10px" }} type="button">Close</button></td></tr>
          </tbody></table>}
